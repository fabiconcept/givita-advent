...
export function FlowerRunner({ canvasRef, scRef, hiRef, fundsRef, progRef }: FlowerRunnerProps) {
  useEffect(() => {
    ...
    const bee = { x: 96, y: 0, vy: 0, j: 0, dead: false, wp: 0 };
    ...
    function resize() {
      ...
      zoom = rect.width < 640 ? 1.35 : 1;
      displayScale = (rect.width / W) * zoom;
  ...
  return (
    <canvas
      ref={canvasRef}
      className="max-sm:aspect-[680/440]"
      style={{ display: 'block', width: '100%', height: 'auto', aspectRatio: '680/224', cursor: 'pointer', touchAction: 'none' }}
    />
  );
}
